...
interface ProjectStackRepository : JpaRepository<ProjectStackEntity, Long> {
    ...
    @Query("select distinct s from ProjectStackEntity s left join fetch s.techStack t where s.projectId = :projectId")
    fun findByProjectId(projectId: Long): List<ProjectStackEntity>

    @Query("select distinct s from ProjectStackEntity s left join fetch s.techStack t where s.projectId in :projectIds")
