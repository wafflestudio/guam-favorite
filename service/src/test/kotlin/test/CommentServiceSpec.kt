...
class CommentServiceSpec @Autowired constructor(
    ...
    fun createCommentWithContentOK() {
        ...
        taskRepository.save(ThreadServiceSpec.DefaultInput.task)
        val thread = database.getThread()
        val result = chatService.createComment(
            command = ThreadServiceSpec.DefaultCommand.CreateComment.copy(threadId = thread.id, userId = user.id)
        )
    ...
    fun createCommentWithImagesOK() {
        ...
        for (commentId in listOf(1L, 2L, 3L)) {
            for (imageFile in ThreadServiceSpec.DefaultInput.imageFiles) {
                every {
        ...
        for (i in listOf(1, 2, 3)) {
            chatService.createComment(
                command = ThreadServiceSpec.DefaultCommand.CreateComment.copy(
                    threadId = thread.id,
                    userId = user.id,
                    content = null,
                    imageFiles = ThreadServiceSpec.DefaultInput.imageFiles
                )
            )
        }
    ...
    fun createCommentBlankInputBecomesNullOK() {
        ...
        taskRepository.save(ThreadServiceSpec.DefaultInput.task)
        every {
            imageService.upload(ThreadServiceSpec.DefaultInput.imageFiles[0], ImageInfo(targetCommentId, ImageType.COMMENT))
        ...
        chatService.createComment(
            command = ThreadServiceSpec.DefaultCommand.CreateComment.copy(
                ...
                userId = user.id,
                content = "             ",
                imageFiles = listOf(ThreadServiceSpec.DefaultInput.imageFiles[0])
            )
        )
    ...
    fun createCommentThreadNotFoundException() {
        shouldThrowExactly<DataNotFoundException> {
            chatService.createComment(
                command = ThreadServiceSpec.DefaultCommand.CreateComment.copy(threadId = 9999999)
            )
        }
    }
    ...
    fun createCommentTaskNotFound() {
        ...
        shouldThrowExactly<DataNotFoundException> {
            chatService.createComment(
                command = ThreadServiceSpec.DefaultCommand.CreateComment
            )
        }
    }
    ...
    fun createCommentGuestCommentInOwnerThreadOK() {
        ...
        taskRepository.save(
            ThreadServiceSpec.DefaultInput.task.copy(
                userState = UserState.GUEST
            )
        )
        chatService.createThread(
            command = ThreadServiceSpec.DefaultCommand.CreateThread.copy(projectId = project.id, userId = user.id)
        )
        database.flush()
        val result = chatService.createComment(command = ThreadServiceSpec.DefaultCommand.CreateComment)
    ...
    fun createCommentGuestOtherThreadNotAllowedException() {
        ...
        database.getProject()
        taskRepository.save(
            ThreadServiceSpec.DefaultInput.task.copy(userId = users[0].id, userState = UserState.LEADER)
        )
        val guest = taskRepository.save(
            ThreadServiceSpec.DefaultInput.task.copy(userId = users[1].id, userState = UserState.GUEST)
        )
        val otherThread = database.getThread()
        chatService.createThread(
            command = ThreadServiceSpec.DefaultCommand.CreateThread.copy(userId = guest.userId)
        )
        database.flush()
    ...
    fun createCommentQuitTaskNotAllowedException() {
        ...
        val quitTask = taskRepository.save(
            ThreadServiceSpec.DefaultInput.task.copy(userId = user.id, userState = UserState.QUIT)
        )
        database.flush()
        shouldThrowExactly<NotAllowedException> {
            chatService.createComment(
    ...
    fun editCommentContentOK() {
        ...
        val result = chatService.editCommentContent(
            command = ThreadServiceSpec.DefaultCommand.EditCommentContent.copy(
                commentId = 1,
                userId = user.id,
            )
        )
    ...
    fun deleteCommentImageOK() {
        ...
        taskRepository.save(ThreadServiceSpec.DefaultInput.task)
        val thread = database.getThread()
        chatService.createComment(command = ThreadServiceSpec.DefaultCommand.CreateComment.copy(threadId = thread.id, userId = user.id))
        ...
        chatService.deleteCommentImage(
            command = ThreadServiceSpec.DefaultCommand.DeleteCommentImage.copy(
    ...
    fun deleteCommentNotFoundException() {
        shouldThrowExactly<DataNotFoundException> {
            chatService.deleteComment(
                command = ThreadServiceSpec.DefaultCommand.DeleteComment.copy(
